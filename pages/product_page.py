...
class ProductPage(BasePage):
    ...
    def should_be_message_product_in_basket(self):
        ...
        by_product = self.browser.find_element_by_css_selector(".alert.alert-safe.alert-noicon.alert-success.fade.in .alertinner").text
        assert product in by_product, "by false product"

    def should_be_message_cost_product(self):
        # проверка на совпадения цены покупаемого продукта, с ценой добавленного в корзину
        cost_by_product = self.browser.find_element_by_css_selector(".alert.alert-safe.alert-noicon.alert-info.fade.in .alertinner").text
        cost_product = self.browser.find_element_by_css_selector(".col-sm-6.product_main .price_color").text
        assert cost_product in cost_by_product, "cost in bassket false"
